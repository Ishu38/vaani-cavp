...
export default function App({ onSwitchPublic }) {
  // Legacy school-pilot console — always renders the legacy UI now (route-mounted at /legacy).
  // Toggling back to the public IELTS/TOEFL UI is handled by the parent router via onSwitchPublic.
  const [tpMode, setTpMode] = useState(false);
  const enterLegacy = useCallback(() => { setTpMode(false); }, []);
  ...
  // tpMode is permanently false here — public UI is mounted under "/" by the router.
  // The "Back to public" button in the legacy chrome calls onSwitchPublic to navigate away.
